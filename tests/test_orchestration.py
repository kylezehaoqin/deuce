...
# Skipping locally is a convenience; skipping in CI is a lie. The manifest is
# generated by a CI step, so its absence there means the pipeline is broken, not
# that the check is inapplicable -- and a silent skip reports as "27 passed,
# 4 skipped", which reads green. So: skip locally, FAIL under CI.
_IN_CI = os.environ.get("CI") == "true"

if not MANIFEST.exists() and _IN_CI:
    raise RuntimeError(
        f"{MANIFEST} missing under CI. The 'Generate dbt manifest' step must run "
        "before the tests, or the orchestration seam goes unchecked."
    )
